...
const generatePages = (context) => new Promise(async (resolve, reject) => {
  try {
    ...
    const { source, destination, templatePath, baseUrl } = config;
    ...
    async.each(mappedPages, async (page, done) => {
      try {
        ...
        switch (page.getFileType()) {
          case 'ejs':
          case 'html':
            renderedPage = ejs.render(templatePage.replace('CONTENT', pageContent), { page, context }, { filename: page.sourceAbsolutePath, cache: false });
            break;
          case 'md':
...
const processAssets = (context) => new Promise(async (resolve, reject) => {
  const { config } = context;
  const { source, destination, assetPath } = config;
  try {
    ...
    async.forEach(files, async (file, done) => {
      ...
      switch (extension) {
        case 'scss':
          sass.render({ file: sourceAbsolutePath, outputStyle: 'expanded' }, async (err, result) => {
            if (err) {
...
class Builder extends EventEmitter {
  ...
  watch() {
    ...
    const sourceWatcher = chokidar.watch(source, { ignoreInitial: true });
    sourceWatcher.on('all', () => runBuild());

    const buildWatcher = chokidar.watch(destination, { ignoreInitial: true });
